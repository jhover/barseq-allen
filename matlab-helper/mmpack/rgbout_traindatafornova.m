function rgbout_traindatafornova(string,strength,m,b,shiftmat,chprofile)
...
for k=2
    ...
    if ~isempty(chprofile)
        im=imageC;im(:,:,2)=imageY;%im(:,:,3)=imageM;im(:,:,4)=imageW;
        im=reshape(uint16(double(reshape(im,[],2))/chprofile),size(im,1),size(im,2),2);%subtract camera baseline and correct for bleeding
        imageC=im(:,:,1);
        imageY=im(:,:,2);
        %imageM=im(:,:,3);
        %imageW=im(:,:,4);
...
%     imageC=imageC/2;
    [p,n]=size(imageC);
    imageRGB(1:p,1:n,1:3)=0;
    imageRGB(1:p,1:n,1)=double(imageY)*strength;
    imageRGB(1:p,1:n,2)=double(imageY+imageC)*strength;
    imageRGB(1:p,1:n,3)=double(imageC)*strength;
