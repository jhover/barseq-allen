...
plot_analysis = function(input_dir, output_dir, max_cells = 2000000) {
    ...
    if (!("aligned_x" %in% colnames(colData(barseq)))) {
        barseq$aligned_x<-barseq$pos_x
        barseq$aligned_y<-barseq$pos_y
    }
...
plot_subglu = function(input_dir = "analysis/subglu", output_dir = "figs/subglu") {
 ...
 load_dataset = function(input_dir, max_cells=2000000) {
    ...
    colLabels(barseq) = cluster_annotation[as.vector(barseq$cluster_id)]
...
plot_umap = function(barseq, output_dir, text_size=3, filename="preliminary_analysis.pdf") {
    ...
    p = plot_raster_umap(barseq, color_by="label", text_by="label", text_size=text_size, point_size=point_size) +
...
plot_raster_umap = function(barseq, color_by, text_by=NULL, point_size=0.1, text_size=3, raster.dpi=150) {
    ...
    result=to_plot %>%
        ...
        ggrastr::geom_point_rast(size=point_size, shape=16, stroke=0, raster.dpi = raster.dpi) +
        theme_classic()
