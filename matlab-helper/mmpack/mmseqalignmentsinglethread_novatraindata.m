function [chprofile,tforms,warnmsg]=mmseqalignmentsinglethread_fornovatraindata(filename,chprofile, bgnradius,shiftmat)
...
currentfile=files{1};
[~,fixedfilelist{1}]=fixbleed_Miseq(currentfile,chprofile,bgnradius,shiftmat);
movefile(files{1},['original/',files{1}]);


currentfile=files{2};
[~,fixedfilelist{2}]=fixbleed_Nova(currentfile,chprofile,bgnradius,shiftmat);
movefile(files{2},['original/',files{2}]);
...
%%
% This subfunction fixes bleed through between channels in illumina seq tiff file for Novaseq.
function [chprofile,fixedimage]=fixbleed_Nova(filename,chprofile,radius,shiftmat)
...
% This subfunction fixes bleed through between channels in illumina seq tiff file for Miseq.
...
imagesum=double(sum(im,3))./double(max(max(sum(im,3))));
