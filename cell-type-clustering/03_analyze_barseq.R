...
main = function() {
    ...
    #try out different clustering on L4/5 it
    #barseq = load_barseq(normalization_factor = 10)
    #logcounts(barseq) = log1p(cpm(barseq))/log(2)
    #analyze_IT45(barseq)
}
...
analyze_IT45 = function(barseq) {
  barseq = filter_barseq(barseq, "analysis/glu")
  analyze_barseq(barseq[, startsWith(barseq$label, "IT_2")], "IT45", k_umap=15, k_cluster=15)
}
...
analyze_barseq = function(barseq, output_name, n_pca=30, k_umap=100, k_cluster=50) {
    ...
    dir.create(output_dir, showWarnings = FALSE, recursive=TRUE)
    #For each round, PCA, UMAP and clustering
